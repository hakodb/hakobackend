//! ub-db-hako: adapter HakoDB (driver default hakobackend).
//!
...
use std::sync::Arc;
use ub_core::{AppError, Change, Database, Doc, QueryOptions};
...
fn map_filter(f: &ub_core::Filter) -> Result<hakodb::query::filter::Filter, AppError> {
    use hakodb::query::filter::Operator as HO;
    let op = match f.op {
        ub_core::FilterOp::Eq => HO::Eq,
        ub_core::FilterOp::Ne => HO::Ne,
        ub_core::FilterOp::Gt => HO::Gt,
        ub_core::FilterOp::Lt => HO::Lt,
        ub_core::FilterOp::Gte => HO::Gte,
        ub_core::FilterOp::Lte => HO::Lte,
        ub_core::FilterOp::ArrayContains => HO::ArrayContains,
        ub_core::FilterOp::ArrayContainsAny => HO::ArrayContainsAny,
        ub_core::FilterOp::In => HO::In,
    };
...
impl Database for HakoDb {
    ...
    async fn list(&self, collection: &str, q: &QueryOptions) -> Result<Vec<Doc>, AppError> {
        ...
        for o in &q.order_by {
            query = query.order_by(&o.field, matches!(o.direction, ub_core::Direction::Asc));
        }
        if !emulate {
        ...
        if emulate {
            ...
            Ok(ub_core::conformance::apply_offset_limit(
                ub_core::conformance::apply_cursor(docs, q),
                q,
            ))
        } else {
            Ok(docs)
    ...
    async fn create_index(&self, collection: &str, spec: &ub_core::IndexSpec) -> Result<ub_core::IndexInfo, AppError> {
            ...
            match spec.kind {
                ...
                ub_core::IndexKind::Composite => {
                    ...
                    Ok(ub_core::IndexInfo {
                        ...
                        fields: names,
                        unique: false,
                        kind: ub_core::IndexKind::Composite,
                    })
                }
                ub_core::IndexKind::FullText => {
                    ...
                    Ok(ub_core::IndexInfo {
                        ...
                        fields: vec![f],
                        unique: false,
                        kind: ub_core::IndexKind::FullText,
                    })
                }
            }
        })
    ...
    async fn list_indexes(&self, collection: &str) -> Result<Vec<ub_core::IndexInfo>, AppError> {
        let db = self.inner.clone();
        ...
        tokio::task::spawn_blocking(move || {
            ...
            for fields in list.simple.values().chain(list.secondary.values()).flat_map(|v| v.iter()) {
                out.push(ub_core::IndexInfo {
                    name: fields.clone(),
                    fields: vec![fields.clone()],
                    unique: false,
                    kind: ub_core::IndexKind::Simple,
                });
            }
            ...
            for comp in &list.composite {
                ...
                let logical = ub_core::conformance::auto_index_name(&ub_core::IndexSpec {
                    name: None,
                    fields: fields.clone(),
                    unique: false,
                    kind: ub_core::IndexKind::Composite,
            ...
            match ev.kind {
                hakodb::engine::ChangeKind::Put => {
                    if let Ok(Some(hako)) = db.get(&collection, &id) {
                        ...
                        let kind = if seen.insert(id.clone()) {
                            ub_core::ChangeKind::Add
                        } else {
                            ub_core::ChangeKind::Change
                        };
                ...
                hakodb::engine::ChangeKind::Delete => {
                    ...
                    let _ = tx.send(Change {
                        collection: collection.clone(),
                        id,
                        kind: ub_core::ChangeKind::Remove,
                        old: None,
                        new: None,
                    });
                }
...
mod tests {
    ...
    async fn conformance_hako() {
        let dir = std::env::temp_dir().join(format!("ub_conform_{}", std::process::id()));
