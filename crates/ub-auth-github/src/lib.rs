//! ub-auth-github: verifier-only token GitHub (tanpa alur OAuth).
//!
...
use ub_core::{AppError, AuthProvider, Claims};
...
pub struct GithubOAuth {
    ...
    client: reqwest::Client,
    db: Arc<dyn ub_core::Database>,
}

impl GithubOAuth {
    /// `Ok(None)` = fitur mati (tanpa client id); `Err` = konfigurasi timpang (fail-closed).
    pub fn from_env(db: Arc<dyn ub_core::Database>) -> Result<Option<Arc<Self>>, String> {
...
mod tests {
    ...
    struct FakeDb {
        store: std::sync::Mutex<HashMap<String, HashMap<String, ub_core::Doc>>>,
    }

    #[async_trait::async_trait]
    impl ub_core::Database for FakeDb {
        fn capabilities(&self) -> ub_core::Capabilities {
            ub_core::Capabilities { driver: "fake", supports_watch: false, supports_transactions: false, supports_composite: false, supports_fts: false, supports_drop_index: false, supports_unique: false, supports_named_index: false }
        }
        async fn ensure_collection(&self, _p: &str) -> Result<(), ub_core::AppError> {
            Ok(())
        }
        async fn list_collections(&self) -> Result<Vec<String>, ub_core::AppError> {
            Ok(vec![])
        }
        async fn get(&self, c: &str, id: &str) -> Result<Option<ub_core::Doc>, ub_core::AppError> {
            Ok(self.store.lock().unwrap().get(c).and_then(|t| t.get(id)).cloned())
        }
        async fn list(&self, _c: &str, _q: &ub_core::QueryOptions) -> Result<Vec<ub_core::Doc>, ub_core::AppError> {
            Ok(vec![])
        }
        async fn insert(&self, c: &str, doc: ub_core::Doc) -> Result<ub_core::Doc, ub_core::AppError> {
            self.store.lock().unwrap().entry(c.into()).or_default().insert(doc.id.clone(), doc.clone());
            Ok(doc)
        }
        async fn set(&self, c: &str, id: &str, doc: ub_core::Doc, _m: bool) -> Result<ub_core::Doc, ub_core::AppError> {
            self.store.lock().unwrap().entry(c.into()).or_default().insert(id.into(), doc.clone());
            Ok(doc)
        }
        async fn delete(&self, c: &str, id: &str) -> Result<Option<ub_core::Doc>, ub_core::AppError> {
            Ok(self.store.lock().unwrap().get_mut(c).and_then(|t| t.remove(id)))
        }
        async fn count(&self, _c: &str, _q: &ub_core::QueryOptions) -> Result<u64, ub_core::AppError> {
            Ok(0)
        }
        async fn subscribe(&self, _c: &str) -> Result<tokio::sync::broadcast::Receiver<ub_core::Change>, ub_core::AppError> {
            Ok(tokio::sync::broadcast::channel(1).0.subscribe())
        }
        async fn create_index(&self, _c: &str, _s: &ub_core::IndexSpec) -> Result<ub_core::IndexInfo, ub_core::AppError> {
            Err(ub_core::AppError::BadRequest("fake tanpa index".into()))
        }
        async fn list_indexes(&self, _c: &str) -> Result<Vec<ub_core::IndexInfo>, ub_core::AppError> {
            Ok(vec![])
        }
        async fn drop_index(&self, _c: &str, _n: &str) -> Result<(), ub_core::AppError> {
            Err(ub_core::AppError::BadRequest("fake tanpa index".into()))
        }
    }
    ...
    fn oauth_handle(base: &str, db: Arc<dyn ub_core::Database>) -> Arc<GithubOAuth> {
        GithubOAuth::new(
            "cid-123".into(),
    ...
    async fn oauth_login_url_dan_callback() {
        ...
        let db: Arc<dyn ub_core::Database> = Arc::new(FakeDb { store: std::sync::Mutex::new(HashMap::new()) });
