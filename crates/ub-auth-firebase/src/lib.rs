//! ub-auth-firebase: verifier-only Firebase ID Token (tanpa Admin SDK).
//!
//! Verifikasi RS256 lokal via sertifikat x509 Google (cache 1 jam):
//! `iss == https://securetoken.google.com/<project>`, `aud == <project>`,
//! `exp` valid. Backend tidak menyimpan sesi/user — stateless.
//! Konfig via env: `UB_FIREBASE_PROJECT` (wajib),
//! `UB_FIREBASE_JWKS_URL` (opsional, override untuk test).
...
impl FirebaseVerifier {
    pub fn from_env() -> Result<Arc<dyn AuthProvider>, String> {
        let project_id = std::env::var("UB_FIREBASE_PROJECT")
            .map_err(|_| "auth firebase butuh env UB_FIREBASE_PROJECT".to_string())?;
...
mod tests {
    ...
    /// Env adalah global proses — semua test yang menyentuhnya wajib lewat kunci ini,
    /// dan hanya menahan kunci saat set/from_env/unset (tanpa await di dalamnya).
    static ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

    /// Pasangan kunci RSA-2048 statis khusus test (bukan secret produksi).
    /// Statis agar test cepat — keygen runtime di debug lambat belasan detik.
    ...
    /// Mock server sekali-pakai: layani GET apa pun dengan satu body JSON.
    /// Mengembalikan URL + handle join. Soket blocking + baca sampai header
    /// lengkap (bukan single-read — lihat ub-auth-github).
    ...
    #[tokio::test]
    async fn verify_end_to_end_dan_cache() {
        ...
        // Token salah-kunci / salah-aud / kid asing → Err (chain lanjut).
        ...
        // Verifikasi kedua tidak memukul server lagi (cache) → hits == 1.
