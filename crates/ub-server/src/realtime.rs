...
use ub_core::{AppError, AuthContext, ChangeKind, Database, Doc, Method, QueryOptions};
use ub_policy::PolicyFile;
...
fn matches_full(doc: &Doc, q: &QueryOptions) -> bool {
    ub_core::conformance::doc_matches(doc, &q.filters) && ub_core::conformance::matches_cursor(doc, q)
}
...
async fn run_source(
    ...
    if watch {
        ...
        loop {
            let mut got: Option<(usize, ub_core::Change)> = None;
...
mod tests {
    ...
    fn opts() -> QueryOptions {
        QueryOptions {
            filters: vec![ub_core::Filter {
                field: "age".into(),
                op: ub_core::FilterOp::Gte,
    ...
    fn cocok_penuh_filter_dan_cursor() {
        ...
        let mut qc = opts();
        qc.order_by.push(ub_core::OrderBy {
            field: "age".into(),
            direction: ub_core::Direction::Asc,
        });
