...
pub struct Args {
    /// File config (TOML). Default: ./ub.toml bila ada.
    #[arg(long)]
...
impl IndexDecl {
    pub fn validate(&self) -> Result<ub_core::IndexSpec, String> {
        ...
        let kind = match self.kind.as_str() {
            "simple" => ub_core::IndexKind::Simple,
            "composite" => ub_core::IndexKind::Composite,
            "fts" | "fulltext" => ub_core::IndexKind::FullText,
            other => return Err(format!("[[indexes]] kind `{other}` tak dikenal (simple|composite|fts)")),
        };
        Ok(ub_core::IndexSpec {
...
/// Path config: --config > UB_CONFIG > ./ub.toml (bila ada) > tanpa file.
pub fn config_path(args: &Args) -> Option<String> {
    ...
    let def = "./ub.toml";
    std::fs::metadata(def).ok().map(|_| def.to_string())
}
...
pub fn validate(cfg: &UbConfig) -> Result<String, String> {
    ...
    if let Some(r) = &cfg.rules {
        ub_policy::PolicyFile::load(r)?;
    }
    if let Some(a) = &cfg.auth {
        let spec = ub_auth_core::AuthSpec::parse(a);
        if let ub_auth_core::AuthSpec::File(p) = &spec {
            ub_auth_core::CustomAuth::load(p)?;
        }
    }
...
mod tests {
    ...
    fn presedensi_flag_menang_atas_file() {
        let f = write_tmp("ub_cli_test.toml", "port = 1111\ndriver = \"hako\"\n");
        let mut a = args();
    ...
    fn validate_menolak_driver_asing() {
        ...
        a.config = Some(write_tmp("ub_empty_test.toml", ""));
        let cfg = resolve(&a);
    ...
    fn indexes_decl_validasi() {
        ...
        assert_eq!(cfg.indexes[0].validate().unwrap().kind, ub_core::IndexKind::Composite);
