//! ub-server: gateway HTTP universal (Axum).
//! Wire-protocol kompatibel dengan rethink-firestore/backend agar SDK lama tetap jalan.
//!
//! Plug-and-play database: driver dipilih di `ub.toml` (`database.driver`).
//! Ganti/pasang-lepas DB = edit config + `POST /api/admin/reload` (tanpa rebuild).
//! Tambah driver baru = crate `ub-db-*` yang impl `ub_core::Database` + 1 arm di `open_driver`.
//!
...
use ub_auth_core::{AuthChain, AuthSpec, CustomAuth, open_chain};
use ub_auth_github::GithubOAuth;
use ub_auth_local::{ACCESS_COOKIE, DpopMode, DpopRequest, LocalAuth, REFRESH_COOKIE};
use ub_core::{AuthContext, AuthProvider, Database, Doc, Method, PathKind, QueryOptions, parse_collection_path};
use ub_db_hako::HakoDb;
use ub_db_postgres::PgDb;
use ub_db_sqlite::SqliteDb;
use ub_db_mysql::MysqlDb;
use ub_policy::{Identity, PolicyFile};
use ub_ratelimit::{Limiter, Quota};

#[derive(Clone)]
struct AppState {
    /// Router DB: koleksi -> driver. Hari ini 1 driver untuk semua koleksi;
    /// peta ini yang memungkinkan override per-koleksi (`routes` di ub.toml, fase 3).
    ...
    fn new(path: Option<String>) -> Self {
        match path {
            None => {
                ...
                eprintln!("[ub] WARN: tanpa policy file — semua endpoint TERBUKA (mode dev). Isi `policy_file` di ub.toml untuk produksi.");
                Self {
                    path: None,
...
fn open_auth_result(
    ...
    if let (Some(d), Some(l)) = (custom.as_ref().and_then(|c| c.dpop.as_deref()), &local) {
        ...
        l.set_dpop_mode(ub_auth_local::DpopMode::parse(d)?);
    }
...
fn parse_index_spec(body: &serde_json::Value) -> Result<ub_core::IndexSpec, String> {
...
fn session_cookies(local: &LocalAuth, tokens: &ub_auth_local::SessionTokens) -> HeaderMap {
    let mut h = HeaderMap::new();
    let pair = [
            ...
            match v.get("type").and_then(|t| t.as_str()) {
                ...
                Some("subscribe") => {
                    ...
                    match realtime::subscribe(db, policy, sub_auth, spec).await {
                        ...
                        Err(e) => {
                            let msg = if matches!(e, ub_core::AppError::PermissionDenied) {
...
async fn sse_handler(
    ...
    let sub = match realtime::subscribe(db, policy, auth, realtime::SubSpec { collection, options, group }).await {
        Ok(sub) => sub,
        Err(e) if matches!(e, ub_core::AppError::PermissionDenied) => return forbidden(),
