//! hakobackend-auth-oidc: generic OIDC verifier-only (Keycloak, Auth0, Google, …).
//!
//! Discovery (`{issuer}/.well-known/openid-configuration` → `jwks_uri`) +
//! RS256 verification via JWKS (1-hour cache). `iss` must match; `aud` checked
//! only when `UB_OIDC_AUDIENCE` is set. Stateless, no user store.
//! Env: `UB_OIDC_ISSUER` (required), `UB_OIDC_AUDIENCE` (optional),
//! `UB_OIDC_JWKS_URL` (optional — skip discovery, for tests).
...
mod tests {
    ...
    /// Static test key (same pair as hakobackend-auth-firebase).
    ...
    /// n/e base64url of the key above (for the mock JWKS).
    ...
    /// Mock OIDC: static discovery + JWKS.
    /// Correct HTTP framing (not single-read) + blocking socket — see hakobackend-auth-github.
