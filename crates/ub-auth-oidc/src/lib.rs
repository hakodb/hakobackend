//! ub-auth-oidc: verifier-only OIDC generik (Keycloak, Auth0, Google, …).
//!
...
mod tests {
    ...
    /// Kunci test statis (pasangan yang sama dengan ub-auth-firebase).
    ...
    /// Framing HTTP benar (bukan single-read) + soket blocking — lihat ub-auth-github.
