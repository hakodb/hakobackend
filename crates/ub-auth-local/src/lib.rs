//! ub-auth-local: satu-satunya issuer (fase C). Verifier + manager akun lokal.
//!
...
use std::sync::Arc;
use ub_core::{AppError, AuthContext, AuthProvider, Claims, Database, Doc, Filter, FilterOp, QueryOptions, SessionIssuer};
...
mod tests {
    use super::*;
    use ub_core::{Capabilities, Change, Doc};
    ...
    impl Database for FakeDb {
        fn capabilities(&self) -> Capabilities {
            Capabilities { driver: "fake", supports_watch: false, supports_transactions: false, supports_composite: false, supports_fts: false, supports_drop_index: false, supports_unique: false, supports_named_index: false }
        }
