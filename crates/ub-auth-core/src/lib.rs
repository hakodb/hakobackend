//! ub-auth-core: resolusi `--auth` + rantai verifier + mapping deklaratif.
//!
//! Core tidak mengikat provider apa pun kecuali kontrak (`ub_core::AuthProvider`).
//! Provider eksternal = verifier-only; hanya `local` yang menjadi issuer (fase C).
//! Peran & koleksi user milik user via `ub_policy::Identity`; peran final =
//! **union** peran klaim + peran dokumen user.

use serde::Deserialize;
use std::collections::HashMap;
use std::sync::Arc;
use ub_core::{AuthContext, AuthProvider, Claims, Database};
use ub_policy::Identity;
...
pub fn open_builtin(name: &str) -> Result<Arc<dyn AuthProvider>, String> {
    match name {
        "firebase" => ub_auth_firebase::FirebaseVerifier::from_env(),
        "github" => Ok(ub_auth_github::GithubVerifier::from_env()),
        "oidc" => ub_auth_oidc::OidcVerifier::from_env(),
...
mod tests {
    use super::*;
    use serde_json::json;
    use ub_core::{AppError, Doc, QueryOptions};
    ...
    async fn union_peran_user_doc() {
        ...
        impl Database for FakeDb {
            ...
            async fn subscribe(&self, _c: &str) -> Result<tokio::sync::broadcast::Receiver<ub_core::Change>, AppError> {
                Ok(tokio::sync::broadcast::channel(1).0.subscribe())
            }
            async fn create_index(&self, _c: &str, _s: &ub_core::IndexSpec) -> Result<ub_core::IndexInfo, AppError> {
                Err(AppError::BadRequest("fake tanpa index".into()))
            }
            async fn list_indexes(&self, _c: &str) -> Result<Vec<ub_core::IndexInfo>, AppError> {
                Ok(vec![])
            }
